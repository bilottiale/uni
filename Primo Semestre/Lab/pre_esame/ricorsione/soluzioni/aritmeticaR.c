...
// restituisce il massimo comun divisore (MCD) tra a e b usando 
...
// restituisce il prodotto di tutti gli interi multipli di k
// partendo da n e scendendo fino a n_min (incluso)
// se non ci sono multipli di k nell'intervallo, allora restituisce 1.
int prodotto_multipli_in_intervalloR(int n_min, int n, int k);

// restituisce true se esiste un numero tra n_min ed n (estremi inclusi)
// che e' un divisore di k. restituisce false altrimenti.
...
int somma_dispariR(int n) {
	// caso base:  somma_dispariR(0) = 0
	if (n==0) { 
		return 0;
	}
	// passo induttivo:  
	// somma_dispariR(n) = somma_dispariR(n-1) + (n se n è dispari)
	else { 
		return (n%2==1 ? n : 0) + somma_dispariR(n-1);
	}
}
...
int prodotto_multipli_in_intervalloR(int n_min, int n, int k) {
	...
	// passo induttivo: moltiplica se n e' multiplo di k
	else {
		if (n % k == 0)
			return prodotto_multipli_in_intervalloR(n_min, n-1, k) * n;
		else
...
bool esiste_divisore_in_intervalloR(int n_min, int n, int k) {
	// caso base:
	if (n < n_min) {
		return false; // non e' stato trovato alcun divisore
	}
	else {
		if (k % n == 0)
			return true; // trovato divisore: n divide k
		else 
