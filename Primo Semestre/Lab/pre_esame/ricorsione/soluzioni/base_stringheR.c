...
// e restituisce true se le due stringhe sono uguali e della stessa lunghezza
...
void stampa_strR(const char* pStr) {
	if (*pStr == '\0')
		return;
	// prima stampa, poi prosegui la ricorsione
	printf("%c", *pStr);
	stampa_strR(pStr + 1);
}
...
void stampa_rev_strR(const char* pStr) {
	if (*pStr == '\0')
		return;
	// prima prosegui la ricorsione, poi stampa
...
//------------------------------------------------------------------
bool str_ugualiR(const char* pStrA, const char* pStrB) {
	// se una delle due stringhe e' terminata, allora
	// controlliamo che siano entrambe terminate
	if (*pStrA=='\0' || *pStrB=='\0')
