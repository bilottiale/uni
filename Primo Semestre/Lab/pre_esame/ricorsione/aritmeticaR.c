...
int somma_dispariR(int n) {
	// caso base: somma_dispari(0) = 0
	if(n == 0){
		return 0;
	}
	// passo induttivo: somma_dispari(n) = n + somma_dispari(n-1) se n e' dispari
	else if(n % 2 != 0){
		return n + somma_dispariR(n-1);
	}
	// passo induttivo: somma_dispari(n) = somma_dispari(n-1) se n e' pari
	else{
...
int prodotto_multipli_in_intervalloR(int n_min, int n, int k) {
    // Caso base: se n raggiunge il limite inferiore n_min, ritorna 1
    if (n < n_min) {
        return 1;
    }
    // Se n è un multiplo di k, calcola il prodotto e continua la ricorsione
    if (n % k == 0) {
        return n * prodotto_multipli_in_intervalloR(n_min, n - 1, k);
    }
    // Altrimenti, continua la ricorsione senza includere n nel prodotto
    else {
...
bool esiste_divisore_in_intervalloR(int n_min, int n, int k) {
	// caso base: esiste_divisore_in_intervallo(n, n, k) = true se n e' divisore di k
	if(k % n == 0){
		return true;
	}
	// caso base: esiste_divisore_in_intervallo(n, n, k) = false se n non e' divisore di k
	else if(n == n_min || n < n_min){
		return false;
	}
	// passo induttivo: esiste_divisore_in_intervallo(n_min, n, k) = true se n e' divisore di k
	else{
