...
int main(void) {
    ...
    int buf1[BUF1_LEN];
    size_t buf1_len = 2;
    copia_maggiori_di(A2_LEN, a2, &buf1_len, buf1, 20, 0, 0);
...
bool tutti_pari(const size_t aLen, const int a[]){
    ...
    return tutti_pari(aLen-1, a+1);
}

int somma_elem_dispari(const size_t aLen, const int a[]){
    if(aLen == 0) return 0;
    if(a[0] % 2 != 0) return a[0] + somma_elem_dispari(aLen-1, a+1);
    return somma_elem_dispari(aLen-1, a+1);
}

size_t indice_elem_massimo(const size_t aLen, const int a[]){
    if(aLen == 0) return -1;
    size_t i_max = indice_elem_massimo(aLen-1, a);
    if(i_max == -1 || a[aLen-1] > a[i_max]) return aLen-1;
    return i_max;
}
...
// - un indice di ricorsione i che scorre l'array a[]
// - un indice j che avanza ogni volta che viene copiato un elemento in b[]
void copia_maggiori_di(const size_t aLen, const int a[], size_t* p_bLen, int b[], const size_t val, int i, int j){
    if(i == aLen){
        *p_bLen = j;
        return;
    }
    if(a[i] >= val){
        b[j] = a[i];
        copia_maggiori_di(aLen, a, p_bLen, b, val, i+1, j+1);\
    }
    else{
        copia_maggiori_di(aLen, a, p_bLen, b, val, i+1, j);
    }
}
