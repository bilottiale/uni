...
int main(void) {
    ...
    int arr4[NBUF1] = { 63, 99, 56, 98, 59, 51, 57, 12, 10, 85 };
...
// Restituisce il numero di elementi copiati in pB
size_t copia_nonnegativi(const int* pA, const size_t lenA, 
                         int* pB, const size_t nmaxB)
{
    size_t lenB = 0;
    for (size_t iA=0; iA<lenA && lenB<nmaxB; iA++) {
        if (pA[iA] >= 0) { // elemento da copiare in pB
            pB[lenB] = pA[iA];
            lenB++;
        }
    }
...
// (quindi mantiene solo gli elementi maggiori_di)
// Restituisce il numero di elementi rimasti dopo la riduzione.
size_t filtra_maggiori_di(int* pA, size_t lenA, int maggiori_di) 
{
    size_t j = 0;
    for (size_t i=0; i<lenA; i++) {
...
// Restituisce il numero di elementi inseriti nell'array pB
size_t duplica_pari(const int* pA, const size_t lenA, 
                    int* pB, const size_t nmaxB)
{
    size_t lenB = 0;
...
// Restituisce true se i due array pA e pB sono uguali, cioè se
...
bool array_equal(const int* pA, const size_t lenA,
                 const int* pB, const size_t lenB)
{
    bool equal = (lenA == lenB);
    if (equal) {
        for (size_t i=0; i<lenA && equal; i++) {
            equal = (pA[i] == pB[i]);
        }
    }
    return equal;
