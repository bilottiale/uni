...
public class Laboratorio extends Aula {
    ...
    public Laboratorio(int n_aula, int capienza, List<Integer> accessoriList) {
        super(n_aula, capienza, TipoAula.LABORATORIO);
        this.accessori = EnumSet.noneOf(AccessorioLaboratorio.class);
        
        if(accessoriList != null && !accessoriList.isEmpty()) {
            if(accessoriList.get(0) == 1) {
                this.accessori.add(AccessorioLaboratorio.PC);
            }
            if(accessoriList.size() > 1 && accessoriList.get(1) == 1) {
                this.accessori.add(AccessorioLaboratorio.PRESE);
            }
        }
    }
