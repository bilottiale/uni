...
BEGIN
    ...
    END IF;
    INSERT INTO ordine(cliente, prezzo, quantità, stato, id_amministratore, team)
    VALUES(cliente_input, prezzo_input, quantita_input, stato_input, id_amm_input, team_input);
END;
$$;


CALL crea_ordine(
    'Trenitalia S.p.A.'::VARCHAR,
    10000::INTEGER,
    100::INTEGER,
    'validato'::stato_ordine,
    'amm01'::VARCHAR,
    1::INTEGER
);
...
CALL registra_installazione(3, 'BNCLRA88B55F105F', 'Milano', 'Bologna');
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM ordine WHERE id = ordine_id_input AND cliente = cliente_input
    ) THEN
