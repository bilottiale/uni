...
BEGIN
    sigla_emittente := upper(substring(NEW.ente_emittente from 1 for 3));
    NEW.id_certificato := 'CERT-' || NEW.prodotto_id || '-' || to_char(NEW.data_rilascio, 'YYYYMMDD') || '-' || sigla_emittente;
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;

-- Crea il trigger collegato alla tabella certificato
CREATE TRIGGER trigger_genera_id_certificato
