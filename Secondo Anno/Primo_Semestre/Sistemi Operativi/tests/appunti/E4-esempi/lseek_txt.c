...
int main(int argc, char *argv[]) {
	...
	/* il campo che ci serve è st_size */
	size = sb.st_size;

	/* Riposizioniamo l'offset del file a size/2 */
	if ((offset = lseek(fd, (off_t) size/2, SEEK_SET)) == -1) {
		perror("lseek");
		exit(EXIT_FAILURE);
	}
