...
ggplot(stat_tempi, aes(x = Compound, y = Media, fill = Compound)) + 
  ...
  labs(title = "Tempi medi sul giro per mescola", x = "Mescola", y = "Tempo medio (sec)") + 
...
# ------------------------------
ggplot(dati %>% filter(LapTimeSeconds <= 150), aes(x = LapTimeSeconds, fill = Compound)) + 
...
# ------------------------------
ggplot(dati %>% filter(LapTimeSeconds <= 200), aes(x = LapTimeSeconds, fill = Compound)) +
...
# ------------------------------
ggplot(dati %>% filter(LapTimeSeconds <= 150), aes(x = LapTimeSeconds, color = Compound)) +
  stat_ecdf(geom = "step") +
...
# ------------------------------
ggplot(dati %>% filter(LapTimeSeconds <= 150), aes(x = LapTimeSeconds)) +
...
# ------------------------------
ggplot(dati %>% filter(LapTimeSeconds <= 150), aes(x = Compound, y = LapTimeSeconds, fill = Compound)) +
  geom_boxplot() +
...
# ------------------------------
ggplot(dati %>% filter(LapTimeSeconds <= 150), aes(x = Humidity, y = LapTimeSeconds)) +
  # Punti con colore e trasparenza
  ...
  labs(
    title = "Relazione tra Umidità e Tempi sul Giro (<= 150s)",
    subtitle = "Analisi dei dati con un modello di regressione lineare",
    x = "Umidità (%)",
    y = "Tempo sul Giro (s)"
  ) +
  # Limiti personalizzati (se necessario)
  lims(x = c(30, 95), y = c(80, 150)) +
  # Tema personalizzato
...
dati <- read_csv("~/Desktop/uni/Secondo Anno/Statistica/presentazione/spam/datas/filled/all_filled.csv")
...
circuito <- "Italian Grand Prix"  # Sostituisci con il nome del circuito
anno <- 2024  # Sostituisci con l'anno del Gran Premio

dati_filtrati <- dati %>% filter(Driver == pilota & EventName == circuito & Year == anno)

correlazione <- cor(dati_filtrati$LapTimeSeconds, dati_filtrati$TrackTemp, use = "complete.obs")
print(paste("Correlazione tra TrackTemp e LapTimeSeconds per", pilota, "HAM", circuito, "è:", correlazione))
...
# ------------------------------
ggplot(dati %>% filter(LapTimeSeconds <= 150), aes(x = LapTimeSeconds, fill = as.factor(Rainfall))) +
  geom_density(alpha = 0.5) +
...
dati_2023 <- dati %>% filter(Year == 2023)

mediana <- median(dati_2023$LapTimeSeconds, na.rm = TRUE)
...
prob_soft <- dati_2023 %>%
  filter(Compound == "HARD") %>%
  summarise(prob = mean(LapTimeSeconds < mediana, na.rm = TRUE))

print(paste("Probabilità che un tempo sul giro con mescola HARD sia inferiore alla mediana:", prob_soft$prob))
...
# ------------------------------
dati_filtrati <- dati %>% filter(LapTimeSeconds <= 150)
