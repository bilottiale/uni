...
data_filtered_clean <- data_filtered_2018_2023 %>%
...
ggplot(data = data_2024_hamilton, aes(x = LapNumber)) +
  ...
  geom_point(aes(y = predictions_2024_hamilton-3, color = "Tempi Predetti"), size = 1) +  # Tempi predetti
  geom_line(aes(y = predictions_2024_hamilton-3, color = "Tempi Predetti"), alpha = 0.6) +  # Collegare i punti dei tempi predetti
  scale_color_manual(
    name = "Legenda",
