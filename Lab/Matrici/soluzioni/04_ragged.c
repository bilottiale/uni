...
// restituisce la somma di tutti gli elementi sulla riga r della 
// matrice ragged mat.
int somma_righe(const int mat[ROWS][COLS],
                const size_t rags[ROWS],
                const size_t r);

// restituisce il numero di elementi inizializzati nella colonna c
...
// della matrice ragged in input. Se la colonna è vuota, restituisce
// il valore 1.
int prod_colonna(const int mat[ROWS][COLS],
                 const size_t rags[ROWS],
                 const size_t c);

// restituisce il minimo elemento della colonna c.
// restituisce il massimo intero rappresentabile con un int 
...
{
    for (size_t r=0; r<ROWS; r++) {
        bool esiste_primo = false;
        for (size_t c=0; c<rags[r] && !esiste_primo; c++) {
