...
int somma_elem_dispari(const size_t aLen, const int a[]){
    if(aLen==0){
        ...
        return a[0] + somma_elem_dispari(aLen-1, a+1);
    } else {
...
size_t indice_elem_massimo(const size_t aLen, const int a[]){
  if(aLen==0){
      ...
      size_t max = indice_elem_massimo(aLen-1, a+1);
      if(a[0]>a[max]){
          return 0;
      } else {
          return max+1;
...
void copia_maggiori_di(const size_t aLen, const int a[], size_t *p_bLen, int b[], const size_t val){
    if(aLen>0){
        if(a[0]>=val){
            b[0] = a[0];
            copia_maggiori_di(aLen-1, a+1, p_bLen, b+1, val);
            *p_bLen = *p_bLen + 1;
        } else {
            copia_maggiori_di(aLen-1, a+1, p_bLen, b, val);
        }
    }
};
